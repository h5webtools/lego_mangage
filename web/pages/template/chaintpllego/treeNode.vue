<template>
  <ul v-if="editData.length > 0">
    <li v-for="(chain, chainIndex) in editData" :key="chain.id">
      <div class="node" :title="chain.chainDes" v-bind:class="{action: chain.nodeType=='action', rule: chain.nodeType=='rule'}">
        {{chain.chainDes}}
      ...
      <ul v-if="chain.nodeType=='rule'">
        <li v-for="(param, index) in chain.params" :key="param.id">
          <div v-if="chain.nodeType == 'rule'">
            ...
            <ul class="inner">
              <li>
                <div  :parent="chain.chainName" tag="match" :chainIndex="chainIndex" :paramIndex="index" class="node match" title="新增满足条件的子节点">符合
                  <i class="glyphicon glyphicon-step-forward"></i>
                </div>
                <tree-node :path="getPath(param)" :parentData="param" :editData="param.match"></tree-node>
              </li>
              <li>
                <div  :parent="chain.chainName" tag="notmatch" class="node match" title="新增不满足条件的子节点">不符合
                  <i class="glyphicon glyphicon-step-forward"></i>
                </div>
