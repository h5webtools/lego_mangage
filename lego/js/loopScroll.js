...
define(function (require, exports, module) {
	...
	$.extend(scroll.prototype, {
		...
		startEvent : function() {
			...
			if (this.loopScroll) {
				var first = this.moveChild.eq(0).clone().attr({id:'',onload:''});
				first.find('img').attr({id:'',onload:''});
				this.moveDom.append(first);
				var last = this.moveChild.eq(this.len - 1).clone().attr({id:'',onload:''});
				last.find('img').attr({id:'',onload:''});
		...
		touchmove : function(e) {
			...
			//if (Math.abs(x) > Math.abs(y)) {
                ...
                this.moveDom.css({
                    ...
                    "-webkit-transform" : this.enableTransX ? "translateX(" + (this.loopScroll?this.index:this.index-1)*(-this.transXVal) + "%)" :
		...
		stepMove : function(no, isSetOffsetIndex) {
            ...
            this.moveDom.css({
                "-webkit-transform" : this.enableTransX ?
                    "translateX(" + (this.loopScroll?no:this.index-1)*(-this.transXVal) + "%)" :"translate3D(" + tran + "px,0,0)" ,
