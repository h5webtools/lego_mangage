...
class LegoEditNpmController extends Controller {
  async savePage() {
    ...
    try {
      ...
      this.ctx.logger.info('更新活动配置失败'+ e.message);
      this.ctx.body = {
  ...
  async getPage() {
    ...
    try {
      let data;
      if(raw.pageId) {
        data = await this.service.lego.legoV2Service.getPage({
          pageId: raw.pageId,
        });
      ...
      this.ctx.logger.info('获取活动配置失败'+ e.message);
      this.ctx.body = {
        code: QUERY_DATABASE_FAILED,
        msg: e.message()
      }
...
module.exports = LegoEditNpmController;