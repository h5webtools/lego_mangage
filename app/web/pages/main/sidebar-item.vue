    ...
    <template v-for="item in menusList">
      <!-- <router-link v-if="item.menu_items.length == 0" :to="item.path"> -->
        <el-menu-item v-if="item.children.length == 0" :index="item.menu_url">
          <router-link :to="item.menu_url"><span slot="title">{{item.menu_name}}</span></router-link>
        </el-menu-item>
      <!-- </router-link> -->
      <el-submenu :index="item.menu_name" v-if="item.children.length > 0">
        ...
        <template v-for="child in item.children">
          ...
          <el-menu-item v-else :index="child.menu_url">
            <span slot="title"><router-link class="menu-link" :to="child.menu_url">{{child.menu_name}}</router-link></span>
            <!-- <a class="menu-link" :href="'#'+child.menu_url"><span slot="title">{{child.menu_name}}</span></a> -->
          </el-menu-item>
            <!-- </router-link> -->
        </template>
      </el-submenu>
